...
# Router Fallback & Rule-based Routing
VALID_ROUTER_TYPES = {
    "general-query",
    "additional-query",
    "graphrag-query",
...
DEFAULT_ROUTER: Dict[str, str] = {
    "type": "additional-query",
    "logic": "Router fallback: invalid or empty classifier output."
}
# 电商/智能家居高频商品词
PRODUCT_KEYWORDS = {
    "空调", "智能空调", "冰箱", "智能冰箱", "洗衣机", "智能洗衣机",
    "扫地机器人", "摄像头", "智能摄像头", "门锁", "智能门锁",
    "灯泡", "智能灯泡", "灯带", "开关", "智能开关",
    "音箱", "智能音箱", "遥控器", "温控器", "传感器",
    "门铃", "猫眼", "集线器", "路由器", "净化器"
}
...
IMAGE_QUERY_HINTS = {"图片", "照片", "截图", "这张图", "帮我看看图"}
FILE_QUERY_HINTS = {"文件", "文档", "附件", "pdf", "word", "excel", "表格"}

def _normalize_router_output(response: Any) -> Dict[str, str]:
    """
    统一把模型返回的 Router / Pydantic对象 / dict / None 转成 dict
    """
    if response is None:
        return DEFAULT_ROUTER.copy()
    ...
    if isinstance(response, dict):
        ...
        if router_type in VALID_ROUTER_TYPES:
            return {
                "type": router_type,
                "logic": logic or "Structured router output.",
                "question": question
            }
        return DEFAULT_ROUTER.copy()
    ...
    # 普通对象属性兜底
    router_type = getattr(response, "type", None)
    logic = getattr(response, "logic", "")
...
def rule_based_route(query: str) -> Optional[Dict[str, str]]:
    """
    对短query/高频商品词做规则路由，避免完全依赖 LLM
    """
    ...
    if not q:
        return {
            "type": "additional-query",
            "logic": "Rule-based route: empty query."
        }

    # 图片优先
    if any(k in q for k in IMAGE_QUERY_HINTS):
        return {
            "type": "image-query",
            "logic": "Rule-based route: image-related query."
        }

    # 文件优先
    if any(k in q for k in FILE_QUERY_HINTS):
        return {
            "type": "file-query",
            "logic": "Rule-based route: file-related query."
        }

    # 单个/短商品词，直接走 graphrag
    if q in PRODUCT_KEYWORDS:
        return {
            "type": "graphrag-query",
            "logic": "Rule-based route: exact product/category keyword."
        }
    ...
    if any(k in q for k in PRODUCT_KEYWORDS):
        return {
            "type": "graphrag-query",
            "logic": "Rule-based route: matched product/category keyword."
        }

    if any(hint in q for hint in PRODUCT_QUERY_HINTS) and any(k in q for k in PRODUCT_KEYWORDS):
        return {
            "type": "graphrag-query",
            "logic": "Rule-based route: product query with shopping intent."
        }

    # 极短 query（1~4字）且像商品名，默认走 graphrag
    if len(q) <= 4:
        short_product_like_words = {"空调", "冰箱", "门锁", "灯泡", "音箱", "猫眼", "开关"}
        if q in short_product_like_words:
            return {
                "type": "graphrag-query",
                "logic": "Rule-based route: short product-like query."
            }
...
async def analyze_and_route_query(
    ...
    try:
        if state.messages:
            ...
            user_query = getattr(last_message, "content", "") or ""
    except Exception as e:
        logger.warning(f"Failed to get user query from state.messages: {e}")
        user_query = ""

    # 0. 图片上传优先
    image_path = config.get("configurable", {}).get("image_path") if config else None
    if image_path:
        router = {
            "type": "image-query",
            "logic": "Config-based route: image_path detected."
        }
        logger.info(f"Image path detected, bypass router model: {router}")
        return {"router": cast(Router, router)}

    # 1. 规则优先，解决“空调”这类短query不稳定
    rule_router = rule_based_route(user_query)
    if rule_router is not None:
        logger.info(f"Rule-based router hit, query={user_query}, router={rule_router}")
        return {"router": cast(Router, rule_router)}

    # 2. LLM结构化分类
    try:
        response = await model.with_structured_output(Router).ainvoke(messages)
        logger.info(f"Analyze user query type completed, raw result: {response}")

        normalized_router = _normalize_router_output(response)
        logger.info(f"Analyze user query type normalized result: {normalized_router}")

        return {"router": cast(Router, normalized_router)}

    except Exception as e:
        logger.exception(f"Router classification failed: {e}")

        fallback_router = {
            "type": "additional-query",
            "logic": f"Exception fallback in analyze_and_route_query: {str(e)}"
        }
        logger.warning(f"Using fallback router: {fallback_router}")
        return {"router": cast(Router, fallback_router)}

def route_query(
    state: AgentState,
) -> Literal["respond_to_general_query", "get_additional_info", "create_research_plan", "create_image_query", "create_file_query"]:
    """根据查询分类确定下一步操作。"""
    router = getattr(state, "router", None) or {}

    # 防御式读取，避免 NoneType 崩溃
    if isinstance(router, dict):
        _type = router.get("type", "additional-query")
    else:
        _type = getattr(router, "type", "additional-query")

    logger.info(f"Routing query with router={router}, resolved type={_type}")
    ...
    if _type == "general-query":
        ...
        logger.warning(f"Unknown router type {_type}, fallback to get_additional_info")
        return "get_additional_info"
